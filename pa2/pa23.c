        ...
        switch (msg->s_header.s_type) {
            ...
            case TRANSFER: {
                ...
                if (order.s_src == info->id) {
                    ...
                    sprintf(outputBuffer, log_transfer_out_fmt, get_physical_time(), info->id, order.s_amount, order.s_dst);
                    ...
                    sprintf(outputBuffer, log_transfer_in_fmt, get_physical_time(), info->id, order.s_amount, order.s_src);
                    duplicateOutputToTerminal(eventFileLogFd, outputBuffer, 0);
                }
                break;
            }
...
void childProcess(pid_t parent, pid_t self, ProcessInfo *info) {
    ...
    sprintf (message.s_payload, log_started_fmt, get_physical_time(), info->id, self, parent, info->balance);
    ...
    sprintf(outputBuffer, log_received_all_started_fmt, get_physical_time(), info->id);
    duplicateOutputToTerminal(eventFileLogFd, outputBuffer, 0);
    ...
    sprintf (message.s_payload, log_done_fmt, get_physical_time(), info->id, info->balance);
    message.s_header.s_payload_len = strlen(message.s_payload);
    ...
    sprintf(outputBuffer, log_received_all_done_fmt, get_physical_time(), info->id);
    duplicateOutputToTerminal(eventFileLogFd, outputBuffer, 0);
...
int main(int argc, char* argv[]) {
    ...
    fprintf(eventFileLogFd, log_received_all_started_fmt, get_physical_time(), info.id);
    fflush(eventFileLogFd);
    ...
    fprintf(eventFileLogFd, log_received_all_done_fmt, get_physical_time(), info.id);
    fflush(eventFileLogFd);
